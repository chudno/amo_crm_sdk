...
// Value представляет значение пользовательского поля
type Value struct {
...
// Field представляет структуру пользовательского поля
type Field struct {
	ID           int    `json:"id"`
...
// Enum представляет вариант значения для поля типа список
type Enum struct {
	ID    int    `json:"id"`
