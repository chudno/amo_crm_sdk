...
* Поддержка `context.Context` во всех публичных функциях
* Работа со всеми основными сущностями amoCRM (сделки, контакты, компании, задачи, воронки, примечания, теги и т.д.)
...
Интеграционные тесты выполняют реальные запросы к API amoCRM. Для их запуска необходимо задать переменные окружения:
